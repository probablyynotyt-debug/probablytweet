#!/bin/bash
cat << 'INNER_EOF' > /tmp/profile_main_changes.txt
...
export const Profile = () => {
  ...
  const { tweets: myTweets, loadingProfile, profile, setProfile } = useTweets(handle);
...
INNER_EOF
cp /tmp/profile_main_changes.txt src/pages/Profile.tsx
